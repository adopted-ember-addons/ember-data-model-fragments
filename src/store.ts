...
import type { FragmentType, RegisteredFragment } from './-private/types.ts';
...
export default class FragmentStore extends Store {
   ...
   * @public
   */
  createCache(storeWrapper: any): any {
   ...
   * @public
   */
  teardownRecord(record: any): void {
  ...
  /**
    ...
    @public
  */
  createFragment<K extends FragmentType>(
    modelName: K,
    ...
    if (macroCondition(dependencySatisfies('ember-data', '>=5.8.0'))) {
      const record = (this as any)._instanceCache.getRecord(identifier);
    ...
    return (this as any)._instanceCache.getRecord(identifier, props);
  }
  ...
  */
  ...
  /**
    ...
    @private
  */
  _fragmentSerializerFor(modelName: string): any {
    ...
    if (!owner.hasRegistration(FALLBACK_KEY)) {
      const FragmentSerializer = (
        importSync('./serializers/fragment.ts') as any
      ).default;
  ...
  /**
    ...
    @private
  */
  _isFragmentSafe(modelName: string): boolean {
    if (
      !modelName ||
  ...
  */
  isFragment(modelName: string): boolean {
    ...
    return (Fragment as any).detect(type);
  }
}

...
class FragmentBehavior {
  declare stateManager: FragmentStateManager;
...
class FragmentArrayBehavior {
  ...
  pushData(fragmentArray: any, canonical: any): any {
    ...
    return canonical.map((attributes: any, i: number) => {
  ...
  didCommit(fragmentArray: any, canonical: any): any {
    ...
    const result = canonical.map((attributes: any, i: number) => {
      const fragment = fragmentArray?.[i];
      if (fragment) {
...
export default class FragmentStateManager {
  declare __storeWrapper: any;
  declare __fragmentData: Map<string, Record<string, any>>;
  declare __fragments: Map<string, Record<string, any> | null>;
  declare __inFlightFragments: Map<string, Record<string, any> | null>;
  declare __fragmentOwners: Map<string, FragmentOwnerInfo>;
  declare __fragmentArrayCache: Map<string, Record<string, any>>;
  declare __behaviors: Map<string, Record<string, Behavior>>;
  declare __committedFragments: Set<string>;
  declare __inFlightAttrValues: Map<string, Record<string, any>>;
  ...
  _identifierFor(fragmentOrRecord: any): FragmentIdentifier {
    ...
    return recordIdentifierFor(fragmentOrRecord) as FragmentIdentifier;
  }
  ...
  _getBehaviors(identifier: FragmentIdentifier): Record<string, Behavior> {
    ...
    behaviors = Object.create(null) as Record<string, Behavior>;
  ...
  _getFragmentDataMap(identifier: FragmentIdentifier): Record<string, any> {
    ...
    if (!data) {
      data = Object.create(null) as Record<string, any>;
  ...
  hasFragment(identifier: FragmentIdentifier, key: string) {
    const fragments = this.__fragments.get(identifier.lid);
    if (fragments && key in fragments) {
      return true;
    }
  ...
  valueContainsFragmentInstance(value: any): boolean {
    ...
    if (isArray(value)) {
      return (value as any[]).some((item: any) => isFragment(item));
    }
    return false;
  }
  ...
  adoptFragmentForKey(
    ...
    // (raw object array).
    if (!(value as any[]).some((item: any) => isFragment(item))) {
      return undefined;
    }
  ...
  changedFragments(identifier: FragmentIdentifier): Record<string, any> {
    const diffData = Object.create(null) as Record<string, any>;
    const behaviors = this._getBehaviors(identifier);
    const fragments = this.__fragments.get(identifier.lid) || {};
    const inFlight = this.__inFlightFragments.get(identifier.lid) || {};
  ...
  pushFragmentData(
    ...
    let changedFragmentKeys: string[] | undefined;
    const behaviors = this._getBehaviors(identifier);
  ...
  rollbackFragments(identifier: FragmentIdentifier): string[] {
    let dirtyFragmentKeys: string[] | undefined;
    const fragments = this.__fragments.get(identifier.lid);

...
function isFragmentAttribute(meta) {
  return meta && meta.isFragment && (meta.kind === 'fragment' || meta.kind === 'fragment-array' || meta.kind === 'array');
}
 ...
 * @private
 */
function fragmentSerialize(serializer, snapshot, payload) {
  ...
  modelClass.eachComputedProperty((key, meta) => {
    ...
    const transform = serializer.transformFor(meta.type);
    attributes[attributeKey] = transform.serialize(value, meta.options, snapshot);
  });
  return payload;
}
 ...
 * @private
 */
function fragmentExtractAttributes(serializer, modelClass, resourceHash, superExtractAttributes) {
  // First, call parent to get regular attributes
  const attributes = superExtractAttributes.call(serializer, modelClass, resourceHash);
 ...
 * @private
 */
function fragmentExtractAttributesJSONAPI(serializer, modelClass, resourceHash, superExtractAttributes) {
  // First, call parent to get regular attributes
  const attributes = superExtractAttributes.call(serializer, modelClass, resourceHash);
...
export { fragmentApplyTransforms, fragmentExtractAttributes, fragmentExtractAttributesJSONAPI, fragmentSerialize, fragmentTransformFor };

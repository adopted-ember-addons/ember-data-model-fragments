...
import { isFragment, getActualFragmentType } from '../fragment.js';
...
function diffArray(oldArray, newArray) {
  ...
  if (oldArray.length !== newArray.length) {
    return {
      firstChangeIndex: Math.min(oldArray.length, newArray.length)
    };
  }
...
class FragmentBehavior {
  ...
  getDefaultValue(key) {
    ...
    assert("The fragment's default value must be an object or null", defaultValue === null || typeOf(defaultValue) === 'object' || isFragment(defaultValue));
    if (defaultValue === null) {
      return null;
    }
    if (isFragment(defaultValue)) {
      assert(`The fragment's default value must be a '${this.definition.modelName}' fragment`, isInstanceOfType(this.stateManager.__storeWrapper._store.modelFor(this.definition.modelName), defaultValue));
      const fragmentIdentifier = this.stateManager._identifierFor(defaultValue);
      this.stateManager.setFragmentOwner(fragmentIdentifier, this.identifier, key);
      return fragmentIdentifier;
    }
    return this.stateManager._newFragmentIdentifier(this.identifier, this.definition, defaultValue);
  }
  pushData(fragment, canonical) {
    assert('Fragment canonical value must be an object or null', canonical === null || typeOf(canonical) === 'object');
    if (canonical === null) {
      return null;
    }
    if (fragment) {
      this.stateManager._fragmentPushData(fragment, {
        attributes: canonical
      });
      return fragment;
    }
    return this.stateManager._newFragmentIdentifier(this.identifier, this.definition, canonical);
  }
  willCommit(fragment) {
    this.stateManager._fragmentWillCommit(fragment);
  }
  didCommit(fragment, canonical) {
    assert('Fragment canonical value must be an object', canonical == null || typeOf(canonical) === 'object');
    if (canonical == null) {
      if (fragment) {
    ...
    if (fragment) {
      this.stateManager._fragmentDidCommit(fragment, {
        attributes: canonical
      });
      return fragment;
    }
  ...
  isDirty(value, originalValue) {
    return value !== originalValue || value !== null && this.stateManager.hasChangedAttributes(value);
  }
  currentState(fragment) {
    return fragment === null ? null : this.stateManager.getCurrentState(fragment);
  }
  canonicalState(fragment) {
    return fragment === null ? null : this.stateManager.getCanonicalState(fragment);
  }
}
...
class FragmentArrayBehavior {
  ...
  getDefaultValue(key) {
    ...
    assert("The fragment array's default value must be an array of fragments or null", defaultValue === null || isArray(defaultValue) && defaultValue.every(v => typeOf(v) === 'object' || isFragment(v)));
    if (defaultValue === null) {
      return null;
    }
    return defaultValue.map(item => {
      if (isFragment(item)) {
        assert(`The fragment array's default value can only include '${this.definition.modelName}' fragments`, isInstanceOfType(this.stateManager.__storeWrapper._store.modelFor(this.definition.modelName), item));
        const fragmentIdentifier = this.stateManager._identifierFor(item);
        this.stateManager.setFragmentOwner(fragmentIdentifier, this.identifier, key);
        return fragmentIdentifier;
      }
      return this.stateManager._newFragmentIdentifier(this.identifier, this.definition, item);
    });
  }
  pushData(fragmentArray, canonical) {
    assert('Fragment array canonical value must be an array of objects', canonical === null || isArray(canonical) && canonical.every(v => typeOf(v) === 'object'));
    if (canonical === null) {
      return null;
    }
    return canonical.map((attributes, i) => {
      ...
      if (fragment) {
        ...
        return this.stateManager._newFragmentIdentifier(this.identifier, this.definition, attributes);
      }
    });
  }
  willCommit(fragmentArray) {
    fragmentArray.forEach(fragment => this.stateManager._fragmentWillCommit(fragment));
  }
  didCommit(fragmentArray, canonical) {
    assert('Fragment array canonical value must be an array of objects', canonical == null || isArray(canonical) && canonical.every(v => typeOf(v) === 'object'));
    if (canonical == null) {
      fragmentArray?.forEach(fragment => this.stateManager._fragmentDidCommit(fragment, null));
      if (canonical === null) {
        return null;
      }
  ...
  commitWasRejected(fragmentArray) {
    fragmentArray.forEach(fragment => this.stateManager._fragmentCommitWasRejected(fragment));
  }
  rollback(fragmentArray) {
    fragmentArray.forEach(fragment => this.stateManager._fragmentRollbackAttributes(fragment));
  }
  unload(fragmentArray) {
    fragmentArray.forEach(fragment => this.stateManager._fragmentUnloadRecord(fragment));
  }
  isDirty(value, originalValue) {
    return !isArrayEqual(value, originalValue) || value !== null && value.some(id => this.stateManager.hasChangedAttributes(id));
  }
  currentState(fragmentArray) {
    return fragmentArray === null ? null : fragmentArray.map(fragment => this.stateManager.getCurrentState(fragment));
  }
  canonicalState(fragmentArray) {
    return fragmentArray === null ? null : fragmentArray.map(fragment => this.stateManager.getCanonicalState(fragment));
  }
}
...
class ArrayBehavior {
  ...
  getDefaultValue(key) {
    ...
    assert("The array's default value must be an array or null", defaultValue === null || isArray(defaultValue));
    if (defaultValue === null) {
      return null;
    }
  ...
  pushData(array, canonical) {
    ...
    assert('Array canonical value must be an array', canonical === null || isArray(canonical));
    if (canonical === null) {
      return null;
    }
  ...
  didCommit(array, canonical) {
    assert('Array value must be an array', array === null || isArray(array));
    assert('Array canonical value must be an array', canonical === null || canonical === undefined || isArray(canonical));
    if (canonical === null) {
      return null;
    }
...
class FragmentStateManager {
  ...
  _getBehaviors(identifier) {
    ...
    const definitions = this.__storeWrapper.getSchemaDefinitionService().attributesDefinitionFor(identifier);
    for (const [key, definition] of Object.entries(definitions)) {
      ...
      const isFragmentAttr = definition.isFragment || definition.options?.isFragment;
      if (!isFragmentAttr) {
        continue;
      }
      ...
      switch (fragmentKind) {
        case 'fragment-array':
          behaviors[key] = new FragmentArrayBehavior(this, identifier, definition);
          break;
        case 'fragment':
  ...
  _getFragmentDefault(identifier, key) {
    ...
    assert(`Attribute '${key}' for model '${identifier.type}' must be a fragment`, behavior != null);
  ...
  setFragmentOwner(fragmentIdentifier, ownerIdentifier, key) {
    assert('Fragments can only belong to one owner, try copying instead', !this.__fragmentOwners.has(fragmentIdentifier.lid) || this.__fragmentOwners.get(fragmentIdentifier.lid).ownerIdentifier === ownerIdentifier);
  ...
  adoptFragmentForKey(ownerIdentifier, key, value) {
    ...
    const behavior = behaviors[key];
    assert(`Attribute '${key}' for model '${ownerIdentifier.type}' must be a fragment`, behavior != null);
    ...
    if (isSingleFragmentBehavior) {
      ...
      assert(`The value provided for fragment attribute '${key}' must be a '${definition.modelName}' fragment`, isInstanceOfType(this.__storeWrapper._store.modelFor(definition.modelName), value));
    ...
    return value.map(item => {
      if (isFragment(item)) {
        assert(`The value provided for fragment array attribute '${key}' can only include '${definition.modelName}' fragments`, isInstanceOfType(this.__storeWrapper._store.modelFor(definition.modelName), item));
  ...
  _newFragmentIdentifierForKey(identifier, key, attributes) {
    ...
    const behavior = behaviors[key];
    assert(`Attribute '${key}' for model '${identifier.type}' must be a fragment`, behavior != null);
    return this._newFragmentIdentifier(identifier, behavior.definition, attributes);
  }
  _newFragmentIdentifier(ownerIdentifier, definition, attributes) {
    const type = getActualFragmentType(definition.modelName, definition.options, attributes, this._getRecord(ownerIdentifier));
    const fragmentIdentifier = this.store.identifierCache.createIdentifierForNewRecord({
      type
    });
    ...
    if (attributes) {
      this.innerCache.upsert(fragmentIdentifier, {
        attributes
      }, false);
    }

    // Process any nested fragment attributes
    this.pushFragmentData(fragmentIdentifier, {
      attributes
    }, false);
    return fragmentIdentifier;
  }
  hasChangedAttributes(identifier) {
    return this.hasChangedFragments(identifier) || this.innerCache.hasChangedAttrs(identifier);
  }
  hasChangedFragments(identifier) {
    const fragments = this.__fragments.get(identifier.lid);
    const inFlight = this.__inFlightFragments.get(identifier.lid);
    // Explicitly return boolean to ensure false instead of undefined
    return Boolean(fragments && Object.keys(fragments).length > 0 || inFlight && Object.keys(inFlight).length > 0);
  }
  getCanonicalState(identifier) {
    ...
    for (const [key, definition] of Object.entries(definitions)) {
      ...
      if (isFragmentAttr) {
        ...
        const behavior = behaviors[key];
        const fragmentData = this._getFragmentDataMap(identifier);
        const value = key in fragmentData ? fragmentData[key] : this._getFragmentDefault(identifier, key);
  ...
  changedFragments(identifier) {
    ...
    const fragments = this.__fragments.get(identifier.lid) || {};
    const inFlight = this.__inFlightFragments.get(identifier.lid) || {};
    const fragmentsOrInFlight = Object.keys(inFlight).length > 0 ? inFlight : fragments;
    const fragmentData = this._getFragmentDataMap(identifier);
    ...
    for (const [key, behavior] of Object.entries(behaviors)) {
      ...
      if (canonicalValue && behavior.isDirty(canonicalValue, canonicalValue)) {
        ...
        diffData[key] = [behavior.canonicalState(canonicalValue), behavior.currentState(canonicalValue)];
      }
    }
    return diffData;
  }
  _changedFragmentKeys(identifier, updates) {
    ...
    for (const key of Object.keys(updates)) {
      ...
      if (eitherIsNull || diffArray(original[key], updates[key]).firstChangeIndex !== null) {
  ...
  pushFragmentData(identifier, data, calculateChange, shouldNotify = true) {
    ...
    if (subFragmentsToProcess.length > 0) {
      ...
      subFragmentsToProcess.forEach(({
        ...
        const current = key in fragmentData ? fragmentData[key] : this._getFragmentDefault(identifier, key);
      ...
      changedFragmentKeys = this._changedFragmentKeys(identifier, newCanonicalFragments);
  ...
  willCommitFragments(identifier) {
    ...
    const fragments = this.__fragments.get(identifier.lid);
    this.__inFlightFragments.set(identifier.lid, fragments || Object.create(null));
  ...
  didCommitFragments(identifier, data) {
    ...
    const changedFragmentKeys = this._changedFragmentKeys(identifier, newCanonicalFragments);
    Object.assign(fragmentData, newCanonicalFragments);
  ...
  _fragmentDidDirty(identifier) {
    ...
    assert(`Fragment '${key}' in owner '${ownerIdentifier.type}' has not been initialized`, key in fragmentData);
  ...
  _fragmentDidCommit(identifier, data) {
    ...
    for (const [key, definition] of Object.entries(definitions)) {
      ...
      const canonicalValue = serverAttrs[key] !== undefined ? serverAttrs[key] : inFlightValues[key];
    ...
    // Notify that attributes changed so computed properties are invalidated
    const allChangedKeys = new Set([...Object.keys(commitAttrs), ...Object.keys(newDirtyAttrs)]);
...
export { FragmentStateManager as default };
//# sourceMappingURL=fragment-state-manager.js.map

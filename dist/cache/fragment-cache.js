...
class FragmentCache {
  ...
  _validateStore() {
    ...
    const hasFragmentMethods = typeof store.createFragment === 'function' && typeof store.isFragment === 'function';
    assert(`ember-data-model-fragments requires your store service to extend FragmentStore.\n\n` + `Create app/services/store.js with the following:\n\n` + `import FragmentStore from 'ember-data-model-fragments/store';\n` + `export default class extends FragmentStore {}\n\n` + `See the ember-data-model-fragments documentation for more information.`, hasFragmentMethods);
  }
  ...
  setFragmentOwner(fragmentIdentifier, ownerIdentifier, key) {
    return this.__fragmentState.setFragmentOwner(fragmentIdentifier, ownerIdentifier, key);
  }
  newFragmentIdentifierForKey(identifier, key, attributes) {
    return this.__fragmentState._newFragmentIdentifierForKey(identifier, key, attributes);
  }
  ...
  put(doc) {
    ...
    if (doc?.content?.data) {
      ...
      } else if (doc.content.data.id != null && typeof doc.content.data.id !== 'string') {
  ...
  _collectFragmentsFromResource(resource, fragmentDataByIdentifier) {
    ...
    const identifier = this.__storeWrapper.identifierCache.getOrCreateRecordIdentifier({
      type: resource.type,
      id: resource.id
    });
    ...
    for (const [key, definition] of Object.entries(definitions)) {
      const isFragment = definition.isFragment || definition.options?.isFragment;
    ...
    if (fragmentKeys.length > 0) {
      fragmentDataByIdentifier.set(identifier, {
  ...
  upsert(identifier, data, hasRecordOrCalculateChanges) {
    ...
    const changedKeys = this.__innerCache.upsert(identifier, data, hasRecordOrCalculateChanges);

    // Handle fragment attributes
    if (fragmentAttributeKeys.length > 0) {
      const changedFragmentKeys = this.__fragmentState.pushFragmentData(identifier, {
        attributes: fragmentData
  ...
  clientDidCreate(identifier, options) {
    ...
    if (options) {
      ...
      const result = this.__innerCache.clientDidCreate(identifier, regularOptions);
      ...
      if (hasFragmentInstanceData) {
        for (const [key, value] of Object.entries(fragmentInstanceData)) {
          const adopted = this.__fragmentState.adoptFragmentForKey(identifier, key, value);
          if (adopted !== undefined) {
            this.__fragmentState.setCanonicalFragmentValue(identifier, key, adopted);
          } else {
      ...
      // canonical-data path.
      if (hasFragmentData) {
        this.__fragmentState.pushFragmentData(identifier, {
          attributes: fragmentData
        }, false, false);
      }
      return result;
  ...
  didCommit(identifier, data) {
    ...
    if (data?.content?.data?.id != null && typeof data.content.data.id !== 'string') {
      data = {
        ...data,
        content: {
          ...data.content,
    ...
    const changedFragmentKeys = this.__fragmentState.didCommitFragments(identifier, fragmentData);
  ...
  hasChangedAttrs(identifier) {
    ...
    return this.isNew(identifier) || this.__innerCache.hasChangedAttrs(identifier) || this.__fragmentState.hasChangedFragments(identifier);
  }
  ...
  rollbackAttrs(identifier) {
    ...
    const dirtyFragmentKeys = this.__fragmentState.rollbackFragments(identifier);
...
export { FragmentCache as default };
//# sourceMappingURL=fragment-cache.js.map

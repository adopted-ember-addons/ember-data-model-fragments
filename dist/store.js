...
/**
  ...
  @public
*/
class FragmentStore extends Store {
  get cache() {
  ...
  createSchemaService() {
    if (macroCondition(dependencySatisfies('ember-data', '>=4.13.0-alpha.0'))) {
      const {
        buildSchema
      } = importSync('@ember-data/model/hooks');
  ...
  teardownRecord(record) {
    ...
    try {
      ...
      if (e?.message?.includes?.('disconnected state') || e?.message?.includes?.('cannot utilize the store')) {
        return;
      }
      throw e;
    }
  ...
  */
  createFragment(modelName, props) {
    ...
    if (macroCondition(dependencySatisfies('ember-data', '>=5.8.0'))) {
      ...
      if (props) {
        const definitions = this.getSchemaDefinitionService().fields(identifier);
  ...
  */
  constructor(...args) {
    super(...args);
    const parentSerializerFor = typeof this.serializerFor === 'function' ? this.serializerFor.bind(this) : null;
  ...
  */
  _fragmentSerializerFor(modelName) {
    ...
    if (!owner.hasRegistration(FALLBACK_KEY)) {
      const FragmentSerializer = importSync('./serializers/fragment.ts').default;
  ...
  */
  _isFragmentSafe(modelName) {
    if (!modelName || modelName === 'application' || modelName === '-default' || modelName.charAt(0) === '-') {
      return false;
    }
    try {
...
export { FragmentStore as default };

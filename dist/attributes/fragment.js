...
import { isFragment, setFragmentOwner, getActualFragmentType } from '../fragment.js';
...
function fragment(type, options) {
  ...
  const cp = computed('currentState', 'hasDirtyAttributes', 'isDestroyed', 'isDestroying', 'store.{_instanceCache,cache}', {
    get(key) {
    ...
    set(key, value) {
      assert('You must pass a fragment or null to set a fragment', value === null || isFragment(value) || typeOf(value) === 'object');
      const identifier = recordIdentifierFor(this);
      const cache = fragmentCacheFor(this.store);
      if (value === null) {
      ...
      if (isFragment(value)) {
        assert(`You can only set '${type}' fragments to this property`, isInstanceOfType(this.store.modelFor(type), value));

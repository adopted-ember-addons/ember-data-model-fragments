...
function isFragmentAttribute(meta) {
  return typeof meta === 'object' && meta !== null && 'kind' in meta && meta.isFragment === true && (meta.kind === 'fragment' || meta.kind === 'fragment-array' || meta.kind === 'array');
}
...
class FragmentSchemaService {
  ...
  doesTypeExist(type) {
    return this._schema.doesTypeExist?.(type) ?? this.hasResource({
      type
    });
  }
  ...
  CAUTION_MEGA_DANGER_ZONE_objectExtensions(field, resolvedType) {
    return this._schema.CAUTION_MEGA_DANGER_ZONE_objectExtensions?.(field, resolvedType);
  }
...
export { FragmentSchemaService as default };

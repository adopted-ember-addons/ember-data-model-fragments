...
const definition = {
  ...
  _normalizeData(data, index) {
    assert(`You can only add '${this.modelName}' fragments or object literals to this property`, typeOf(data) === 'object' || isInstanceOfType(this.store.modelFor(this.modelName), data));
    if (isFragment(data)) {
    ...
    const fragmentIdentifier = this.cache.newFragmentIdentifierForKey(this.identifier, this.key, data);
    return this.store._instanceCache.getRecord(fragmentIdentifier);
  },
  _getFragmentState() {
    ...
    return fragmentIdentifiers?.map(fragmentIdentifier => this.store._instanceCache.getRecord(fragmentIdentifier));
  },
  _setFragmentState(fragments) {
    const fragmentIdentifiers = fragments.map(fragment => recordIdentifierFor(fragment));
  ...
  */
  ...
  createFragment(props) {
    const fragmentIdentifier = this.cache.newFragmentIdentifierForKey(this.identifier, this.key, props);
    if (macroCondition(dependencySatisfies('ember-data', '>=5.8.0'))) {
      ...
      const definitions = this.store.getSchemaDefinitionService().fields(fragmentIdentifier);
      if (props) {
    ...
    const fragment = this.store._instanceCache.getRecord(fragmentIdentifier, props);
    return this.pushObject(fragment);
  }
};
const FragmentArray = StatefulArray.extend(definition);

export { FragmentArray as default };

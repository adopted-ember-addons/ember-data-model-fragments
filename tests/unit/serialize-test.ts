...
module('unit - Serialization', function (hooks) {
  ...
  test('polymorphic properties are deserialized correctly', function (this: TestContext, assert: Assert) {
  ...
  test('fragment properties are snapshotted as normal attributes on the owner record snapshot', async function (this: TestContext, assert: Assert) {
    assert.expect(7);
  ...
  test('fragment properties are snapshotted correctly after fragment wrappers are materialized', async function (this: TestContext, assert: Assert) {
    assert.expect(4);
    ...
    class PersonSerializer extends JSONSerializer {
      serialize(snapshot: any) {
        ...
        assert.deepEqual(
          houses.map((house: any) => house.attr('name')),
          ['Tully', 'Stark'],
    ...
    /* eslint-disable @typescript-eslint/no-unused-expressions -- reading each property is what materializes it */
    person.name;
    person.houses;
    person.children;
  ...
  test('fragment properties are serialized as normal attributes using their own serializers', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test('serializing a fragment array creates a new array with contents the result of serializing each fragment', async function (this: TestContext, assert: Assert) {
    const names = [
      {
        first: 'Rhaegar',
        last: 'Targaryen',
  ...
  test('normalizing data can handle `null` fragment values', function (this: TestContext, assert: Assert) {
  ...
  test('normalizing data can handle `null` fragment values', async function (this: TestContext, assert: Assert) {
    class NullDefaultPerson extends Person {
      @fragmentArray('house', { defaultValue: null }) declare houses: any;
      @array({ defaultValue: null }) declare children: any;
    }
  ...
  test('array properties use the specified transform to normalize data', function (this: TestContext, assert: Assert) {
  ...
  test('array properties use the specified transform to serialize data', async function (this: TestContext, assert: Assert) {
    const values = [1, 0, true, false, 'true', ''];
  ...
  module('models with only fragment attributes (no @attr fields)', function () {
    ...
    test('FragmentJSONAPISerializer writes fragments inside data.attributes', function (this: TestContext, assert: Assert) {
    ...
    test('FragmentRESTSerializer writes fragments under the model root key', function (this: TestContext, assert: Assert) {
      registerFragmentOnly();
    ...
    test('FragmentSerializer (JSONSerializer) writes fragments at the root', function (this: TestContext, assert: Assert) {
      registerFragmentOnly();
    ...
    test('REST/JSON serializers do not treat a top-level `data` @attr as a JSON:API envelope', function (this: TestContext, assert: Assert) {
  ...
  module('when saving the record', function (saveHooks) {
    ...
    test('changedAttributes should have the same result when serialized as before the save is called', async function (this: TestContext, assert: Assert) {
      assert.expect(3);
      ...
      server.put('/components/1', (request: FakeRequest) => {

...
module('unit - `MF.Fragment`', function (hooks) {
  ...
  test('copied fragments can be added to any record', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test("copying a fragment copies the fragment's properties", async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test('fragments are `Comparable`', function (this: TestContext, assert: Assert) {
    const fragment = store.createFragment('name');

    assert.ok(Comparable.detect(fragment), 'fragments are comparable');
  });

  test('fragments support toString', function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'vehicle',
        id: 1,
  ...
  test('fragments are compared by reference', function (this: TestContext, assert: Assert) {
  ...
  test('newly create fragments start in the new state', function (this: TestContext, assert: Assert) {
    const fragment = store.createFragment('name');

    assert.ok(fragment.isNew, 'fragments start as new');
  });

  test("changes to fragments are indicated in the owner record's `changedAttributes`", async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test("fragment properties that are set to null are indicated in the owner record's `changedAttributes`", async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test("fragment properties that are initially null are indicated in the owner record's `changedAttributes`", async function (this: TestContext, assert: Assert) {
  ...
  test('changes to attributes can be rolled back', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test('fragments without an owner can be destroyed', function (this: TestContext, assert: Assert) {
    const fragment = store.createFragment('name');
    fragment.destroy();
    assert.ok(fragment.isDestroying, 'the fragment is being destroyed');
  });

  test('fragments unloaded/reload w/ relationship', async function (this: TestContext, assert: Assert) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions -- reading the property is what primes it
    zoo.star;
  ...
  test('can be created with null', function (this: TestContext, assert: Assert) {
  ...
  test('can be updated to null', function (this: TestContext, assert: Assert) {
    const person = store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  module('fragment bug when initially set to `null`', function (hooks) {
    ...
    test('`person` fragments/fragment arrays are not initially `null`', async function (this: TestContext, assert: Assert) {
    ...
    test('`person` fragments/fragment arrays are initially `null`', async function (this: TestContext, assert: Assert) {
      const person = store.createRecord('person', {
        title: 'Mr.',
        name: null,
        names: null,
  ...
  module('polymorphic', function () {
    module('when updating the type of the model', function () {
      test('it should rebuild the fragment', function (this: TestContext, assert: Assert) {

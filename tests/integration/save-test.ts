...
/** The request object pretender hands to a response handler. */
type FakeRequest = Parameters<ResponseHandler>[0];
...
module('integration - Persistence', function (hooks) {
  ...
  test('persisting the owner record changes the fragment state to non-new', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Viserys',
        last: 'Targaryen',
    ...
    const payload: { person: Record<string, any> } = {
  ...
  test('newly created fragments are not marked as deleted after save', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Viserys',
        last: 'Targaryen',
  ...
  test('persisting the owner record in a clean state maintains clean state', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test('overwrite current state with fragment attributes from the save response', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
    ...
    server.put('/people/1', (request: FakeRequest) => {
  ...
  test('when setting a property to the same value', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test('persisting the owner record when a fragment is dirty moves owner record, fragment array, and all fragments into clean state', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test('persisting a new owner record moves the owner record, fragment array, and all fragments into clean state', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Daenerys',
        last: 'Targaryen',
  ...
  test('a new record can be persisted with null fragments', async function (this: TestContext, assert: Assert) {
    const person = store.createRecord('person');
  ...
  test('the adapter can update fragments on save', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Eddard',
        last: 'Stark',
  ...
  test('the adapter can set fragments to null on save', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Eddard',
        last: 'Stark',
  ...
  test('the adapter can set fragments from null to a new value on save', async function (this: TestContext, assert: Assert) {
    const person = store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test('existing fragments are updated on save', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Eddard',
        last: 'Stark',
  ...
  test('the adapter can update fragments on reload', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Brandon',
        last: 'Stark',
    ...
    /* eslint-disable @typescript-eslint/no-unused-expressions -- reading each property is what primes it */
    person.name.first;
  ...
  */
  test('the adapter can update fragments without infinite loops when CPs are eagerly consumed', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Brandon',
        last: 'Stark',
  ...
  skip('fragment array properties are notified on save', async function (assert) {
    ...
    const person = await store.findRecord('person', 1);
    createClassic(PersonObserver, { person: person });
    return person.save();
  });

  test('fragment array properties are notified on reload', async function (this: TestContext, assert: Assert) {
    ...
    const ArmyObserver = EmberObject.extend({
      army: null,
      observer: observer('army.soldiers.[]', function (this: { army: any }) {
        assert.equal(
    ...
    const payload: { army: Record<string, any> } = {
      army: copy(data, true),
    };
    payload.army.id = 1;
    ...
    createClassic(ArmyObserver, { army: army });
    return army.reload();
  });

  test('string array can be rolled back on failed save', async function (this: TestContext, assert: Assert) {
    // assert.expect(3);
  ...
  test('existing fragments can be rolled back on failed save', async function (this: TestContext, assert: Assert) {
    // assert.expect(3);
  ...
  test('setting an array does not error on save', async function (this: TestContext, assert: Assert) {
    assert.expect(0);
    class Army extends Model {
  ...
  test('change fragment attributes while save is in-flight', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'person',
        id: 1,
  ...
  test('change fragment value while save is in-flight', async function (this: TestContext, assert: Assert) {
    const data = {
      name: {
        first: 'Eddard',
        last: 'Stark',
  ...
  test('initializing a fragment, saving and then updating that fragment', async function (this: TestContext, assert: Assert) {

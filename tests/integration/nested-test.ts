...
module('integration - Nested fragments', function (hooks) {
  ...
  test('`DS.hasManyFragment` properties can be nested', async function (this: TestContext, assert: Assert) {
    const data = {
      info: {
    ...
    const payload: { user: Record<string, any> } = {
      user: copy(data, true),
    };
    payload.user.id = 1;
  ...
  test('Fragments can be created with nested object literals', function (this: TestContext, assert: Assert) {
    const data = {
      info: {
        name: 'Tyrion Lannister',
        notes: ['smart', 'short'],
  ...
  test('Nested fragments can have default values', function (this: TestContext, assert: Assert) {
    const defaultInfo = {
  ...
  test('Nested fragments can be copied', async function (this: TestContext, assert: Assert) {
    const data = {
      info: {
        name: 'Petyr Baelish',
  ...
  test('Nested fragments are destroyed when the owner record is destroyed', async function (this: TestContext, assert: Assert) {
    store.push({
      data: {
        type: 'user',
        id: 1,
